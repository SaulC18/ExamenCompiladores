        ...
        for numero_linea, linea in enumerate(lineas, 1):
            ...
            for token in tokens_linea:
                ...
                if token not in self.reservadas: #si no es una palabra reservada
                    #validacion de los demas tokens para su conteo
            ...
            llaves_abiertas -= linea.count('}')#se resta si se cierra
            parentesis_abiertos += linea.count('(')#se suma si se abre
            parentesis_abiertos -= linea.count(')')#se resta si se cierra
